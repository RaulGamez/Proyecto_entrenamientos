...
export default function Pizarra() {
  ...
  const removeNearestToken = (x, y) => {
    // OJO: aquí usamos potencias, no multiplicación
    const dist2 = (a) => (a.x - x) ** 2 + (a.y - y) ** 2;
  ...
  const createArrowHeadPath = (from, to, size = 9) => {
  ...
  const createStraightPath = (from, to) => {
    const p = Skia.Path.Make();
  ...
  const createDashedPath = (from, to, dash = 12, gap = 8) => {
    const p = Skia.Path.Make();
    const dx = to.x - from.x;
    const dy = to.y - from.y;
  ...
  const createWavyPath = (from, to, waves = 8, amp = 7) => {
    const p = Skia.Path.Make();
    const dx = to.x - from.x;
    const dy = to.y - from.y;
    const len = Math.sqrt(dx * dx + dy * dy) || 1;
  ...
  const handleTouch = (type, e) => {
    ...
    if (tool === "none") {
      if (type === "start") {
        ...
        setStrokes((prev) => [
          ...prev,
          { path: current, color: strokeColor, width: strokeWidth },
        ]);
        setCurrent(null);
      }
    }
      ...
      <View style={styles.screen}>
        ...
        >
          ...
          <ToolToggle
            label="OFF"
            active={tool === "offense"}
            onPress={() => setTool("offense")}
          />
          <ToolToggle
            label="DEF"
            active={tool === "defense"}
            onPress={() => setTool("defense")}
          />
          <ToolToggle
            label="Borrar"
            active={tool === "erase"}
          ...
          <ToolToggle
            label="Bote"
            active={tool === "dribble"}
            onPress={() => setTool("dribble")}
          />

          <ToolToggle
            label="Balón"
            active={tool === "ball"}
            onPress={() => setTool("ball")}
          />
          <ToolToggle
            label="Cono"
            active={tool === "cone"}
            onPress={() => setTool("cone")}
          />
          ...
          {size.w > 0 && size.h > 0 && (
            <Canvas style={{ width: size.w, height: size.h }}>
              ...
              {tempLine &&
                (() => {
                  let basePath;
                  if (tempLine.type === "pass")
                    basePath = createDashedPath(tempLine.from, tempLine.to);
                  else if (tempLine.type === "move")
                    basePath = createStraightPath(tempLine.from, tempLine.to);
...
function ToolToggle({ label, active, onPress }) {
  return (
    <Pressable
      onPress={onPress}
      style={({ pressed }) => [
